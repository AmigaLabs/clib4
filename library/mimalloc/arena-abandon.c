...
static bool mi_arena_segment_os_clear_abandoned(mi_segment_t *segment,
  ...
  if (next != NULL || prev != NULL || subproc->abandoned_os_list == segment) {
    ...
    bool found = false;
    size_t count = 0;
    for (mi_segment_t *current = subproc->abandoned_os_list; current != NULL;
         current = current->abandoned_os_next) {
      if (current == segment) {
        found = true;
      }
      count++;
    ...
    mi_assert_internal(
        count == mi_atomic_load_relaxed(&subproc->abandoned_os_list_count));
#endif
...
bool _mi_arena_segment_clear_abandoned(mi_segment_t *segment) {
  ...
  // reclaim atomically
  bool was_marked = _mi_bitmap_unclaim(arena->blocks_abandoned,
                                       arena->field_count, 1, bitmap_idx);
  if (was_marked) {
  ...
  mi_assert_internal(!was_marked ||
                     _mi_bitmap_is_claimed(arena->blocks_inuse,
                                           arena->field_count, 1, bitmap_idx));
  // mi_assert_internal(arena->blocks_committed == NULL ||
  // _mi_bitmap_is_claimed(arena->blocks_committed, arena->field_count, 1,
  // bitmap_idx));
  return was_marked;
}
...
void _mi_arena_segment_mark_abandoned(mi_segment_t *segment) {
  ...
  mi_atomic_store_release(
      &segment->thread_id,
      (uintptr_t)0); // mark as abandoned for multi-thread free's
  if mi_unlikely (segment->memid.memkind != MI_MEM_ARENA) {
  ...
  mi_subproc_t *const subproc =
      segment->subproc; // don't access the segment after setting it abandoned
  const bool was_unmarked = _mi_bitmap_claim(
      arena->blocks_abandoned, arena->field_count, 1, bitmap_idx, NULL);
  if (was_unmarked) {
    mi_atomic_increment_relaxed(&subproc->abandoned_count);
  }
  mi_assert_internal(was_unmarked);
  mi_assert_internal(_mi_bitmap_is_claimed(arena->blocks_inuse,
                                           arena->field_count, 1, bitmap_idx));
}
...
void _mi_arena_field_cursor_init(mi_heap_t *heap, mi_subproc_t *subproc,
  ...
  const size_t abandoned_count =
      mi_atomic_load_relaxed(&subproc->abandoned_count);
  const size_t abandoned_list_count =
      mi_atomic_load_relaxed(&subproc->abandoned_os_list_count);
  ...
  if (heap != NULL && heap->arena_id != _mi_arena_id_none()) {
    ...
    if (abandoned_count > abandoned_list_count && max_arena > 0) {
      current->start =
          (heap == NULL || max_arena == 0
               ? 0
               : (mi_arena_id_t)(_mi_heap_random_next(heap) % max_arena));
      current->end = current->start + max_arena;
    } else {
      current->start = 0;
      current->end = 0;
    }
    current->os_list_count =
        abandoned_list_count; // max entries to visit in the os abandoned list
  }
...
static mi_segment_t *
mi_arena_segment_clear_abandoned_at(mi_arena_t *arena, mi_subproc_t *subproc,
                                    mi_bitmap_index_t bitmap_idx) {
  // try to reclaim an abandoned segment in the arena atomically
  if (!_mi_bitmap_unclaim(arena->blocks_abandoned, arena->field_count, 1,
                          bitmap_idx))
    return NULL;
  mi_assert_internal(_mi_bitmap_is_claimed(arena->blocks_inuse,
                                           arena->field_count, 1, bitmap_idx));
  mi_segment_t *segment =
      (mi_segment_t *)mi_arena_block_start(arena, bitmap_idx);
  mi_assert_internal(mi_atomic_load_relaxed(&segment->thread_id) == 0);
  // check that the segment belongs to our sub-process
  // note: this is the reason we need the `abandoned_visit` lock in the case
  // abandoned visiting is enabled.
  //  without the lock an abandoned visit may otherwise fail to visit all
  //  abandoned segments in the sub-process. for regular reclaim it is fine to
  //  miss one sometimes so without abandoned visiting we don't need the
  //  `abandoned_visit` lock.
  if (segment->subproc != subproc) {
    // it is from another sub-process, re-mark it and continue searching
    const bool was_zero = _mi_bitmap_claim(
        arena->blocks_abandoned, arena->field_count, 1, bitmap_idx, NULL);
...
static mi_segment_t *
mi_arena_segment_clear_abandoned_next_field(mi_arena_field_cursor_t *previous) {
  const size_t max_arena = mi_arena_get_count();
  size_t field_idx = mi_bitmap_index_field(previous->bitmap_idx);
  size_t bit_idx = mi_bitmap_index_bit_in_field(previous->bitmap_idx);
  // visit arena's (from the previous cursor)
  for (; previous->start < previous->end;
       previous->start++, field_idx = 0, bit_idx = 0) {
    // index wraps around
    size_t arena_idx =
        (previous->start >= max_arena ? previous->start % max_arena
                                      : previous->start);
    mi_arena_t *arena = mi_arena_from_index(arena_idx);
    if (arena != NULL) {
      bool has_lock = false;
      ...
      for (; field_idx < arena->field_count; field_idx++, bit_idx = 0) {
        size_t field =
            mi_atomic_load_relaxed(&arena->blocks_abandoned[field_idx]);
        if mi_unlikely (field != 0) { // skip zero fields quickly
          // we only take the arena lock if there are actually abandoned
          // segments present
          if (!has_lock && mi_option_is_enabled(mi_option_visit_abandoned)) {
            has_lock =
                (previous->visit_all
                     ? (mi_lock_acquire(&arena->abandoned_visit_lock), true)
                     : mi_lock_try_acquire(&arena->abandoned_visit_lock));
            if (!has_lock) {
              if (previous->visit_all) {
                _mi_error_message(
                    EFAULT,
                    "internal error: failed to visit all abandoned segments "
                    "due to failure to acquire the visitor lock");
              }
              // skip to next arena
              break;
            }
          }
          mi_assert_internal(has_lock ||
          ...
          for (; bit_idx < MI_BITMAP_FIELD_BITS; bit_idx++) {
            ...
            if mi_unlikely ((field & mask) == mask) {
              ...
              mi_segment_t *const segment = mi_arena_segment_clear_abandoned_at(
                  arena, previous->subproc, bitmap_idx);
              if (segment != NULL) {
                // mi_assert_internal(arena->blocks_committed == NULL ||
                // _mi_bitmap_is_claimed(arena->blocks_committed,
                // arena->field_count, 1, bitmap_idx));
                if (has_lock) {
                ...
                previous->bitmap_idx = mi_bitmap_index_create_ex(
                    field_idx,
                    bit_idx + 1); // start at next one for the next iteration
                return segment;
              }
            }
          }
...
static mi_segment_t *
mi_arena_segment_clear_abandoned_next_list(mi_arena_field_cursor_t *previous) {
  // go through the abandoned_os_list
  // we only allow one thread per sub-process to do to visit guarded by the
  // `abandoned_os_visit_lock`. The lock is released when the cursor is
  // released.
  if (!previous->hold_visit_lock) {
    previous->hold_visit_lock =
        (previous->visit_all
             ? (mi_lock_acquire(&previous->subproc->abandoned_os_visit_lock),
                true)
             : mi_lock_try_acquire(
                   &previous->subproc->abandoned_os_visit_lock));
    if (!previous->hold_visit_lock) {
      if (previous->visit_all) {
        _mi_error_message(
            EFAULT, "internal error: failed to visit all abandoned segments "
                    "due to failure to acquire the OS visitor lock");
      }
  ...
  while (previous->os_list_count > 0) {
    ...
    mi_lock_acquire(
        &previous->subproc
             ->abandoned_os_lock); // this could contend with concurrent OS
                                   // block abandonment and reclaim from `free`
    mi_segment_t *segment = previous->subproc->abandoned_os_list;
    // pop from head of the list, a subsequent mark will push at the end (and
    // thus we iterate through os_list_count entries)
    if (segment == NULL || mi_arena_segment_os_clear_abandoned(
                               segment, false /* we already have the lock */)) {
...
_mi_arena_segment_clear_abandoned_next(mi_arena_field_cursor_t *previous) {
  if (previous->start < previous->end) {
    // walk the arena
    mi_segment_t *segment =
        mi_arena_segment_clear_abandoned_next_field(previous);
    if (segment != NULL) {
      return segment;
    }
  }
...
bool mi_abandoned_visit_blocks(mi_subproc_id_t subproc_id, int heap_tag,
                               bool visit_blocks, mi_block_visit_fun *visitor,
                               void *arg) {
  // (unfortunately) the visit_abandoned option must be enabled from the start.
  // This is to avoid taking locks if abandoned list visiting is not required
  // (as for most programs)
  if (!mi_option_is_enabled(mi_option_visit_abandoned)) {
    _mi_error_message(EFAULT, "internal error: can only visit abandoned blocks "
                              "when MIMALLOC_VISIT_ABANDONED=ON");
    return false;
  }
  mi_arena_field_cursor_t current;
  _mi_arena_field_cursor_init(NULL, _mi_subproc_from_id(subproc_id),
                              true /* visit all (blocking) */, &current);
  mi_segment_t *segment;
  bool ok = true;
  while (ok &&
         (segment = _mi_arena_segment_clear_abandoned_next(&current)) != NULL) {
    ok =
        _mi_segment_visit_blocks(segment, heap_tag, visit_blocks, visitor, arg);
