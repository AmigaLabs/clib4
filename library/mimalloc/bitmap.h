...
#include "include/mimalloc/types.h"
...
// Create a bit index.
static inline mi_bitmap_index_t mi_bitmap_index_create_ex(size_t idx,
                                                          size_t bitidx) {
  mi_assert_internal(bitidx <= MI_BITMAP_FIELD_BITS);
  return (idx * MI_BITMAP_FIELD_BITS) + bitidx;
}
static inline mi_bitmap_index_t mi_bitmap_index_create(size_t idx,
                                                       size_t bitidx) {
  mi_assert_internal(bitidx < MI_BITMAP_FIELD_BITS);
...
// Like _mi_bitmap_try_find_from_claim but with an extra predicate that must be
// fullfilled
...
bool _mi_bitmap_try_claim(mi_bitmap_t bitmap, size_t bitmap_fields,
...
// Find `count` bits of zeros and set them to 1 atomically; returns `true` on
// success. Starts at idx, and wraps around to search in all `bitmap_fields`
// fields.
bool _mi_bitmap_try_find_from_claim_across(mi_bitmap_t bitmap,
                                           const size_t bitmap_fields,
                                           const size_t start_field_idx,
                                           const size_t count,
                                           mi_bitmap_index_t *bitmap_idx);
...
bool _mi_bitmap_claim_across(mi_bitmap_t bitmap, size_t bitmap_fields,
                             size_t count, mi_bitmap_index_t bitmap_idx,
                             bool *pany_zero, size_t *already_set);
