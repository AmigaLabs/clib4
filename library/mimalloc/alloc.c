...
// Note: in release mode the (inlined) routine is about 7 instructions with a
// single test.
extern inline void *_mi_page_malloc_zero(mi_heap_t *heap, mi_page_t *page,
                                         size_t size,
                                         bool zero) mi_attr_noexcept {
            mi_assert_internal(size >= MI_PADDING_SIZE);
            mi_assert_internal(page->block_size == 0 /* empty heap */ ||
    ...
    // that is tracked by valgrind etc. as non-accessible (through the red-zone,
    // see `mimalloc/track.h`)
    mi_track_mem_undefined(block, mi_page_usable_block_size(page));

    // zero the block? note: we need to zero the full block size (issue #63)
    if mi_unlikely (zero) {
                mi_assert_internal(page->block_size !=
                                   0); // do not call with zero'ing for huge blocks (see
        // _mi_malloc_generic)
    ...
    ptrdiff_t delta =
        ((uint8_t *)padding - (uint8_t *)block - (size - MI_PADDING_SIZE));
#if (MI_DEBUG >= 2)
    mi_assert_internal(delta >= 0 && mi_page_usable_block_size(page) >=
                                         (size - MI_PADDING_SIZE + delta));
#endif
    mi_track_mem_defined(
        padding,
        sizeof(mi_padding_t)); // note: re-enable since mi_page_usable_block_size
                               // may set noaccess
    ...
    if (!mi_page_is_huge(page)) {
      ...
      const size_t maxpad = (delta > MI_MAX_ALIGN_SIZE
                                 ? MI_MAX_ALIGN_SIZE
...
extern void *_mi_page_malloc(mi_heap_t *heap, mi_page_t *page,
                             size_t size) mi_attr_noexcept {
    return _mi_page_malloc_zero(heap, page, size, false);
}

extern void *_mi_page_malloc_zeroed(mi_heap_t *heap, mi_page_t *page,
                                    size_t size) mi_attr_noexcept {
...
mi_heap_malloc_small_zero(mi_heap_t *heap, size_t size,
                          bool zero) mi_attr_noexcept {
    mi_assert(heap != NULL);
    mi_assert(size <= MI_SMALL_SIZE_MAX);
#if MI_DEBUG
    const uintptr_t tid = _mi_thread_id();
    mi_assert(heap->thread_id == 0 ||
              heap->thread_id == tid); // heaps are thread local
#endif
...
_mi_heap_malloc_zero_ex(mi_heap_t *heap, size_t size, bool zero,
    ...
    if mi_likely (size <= MI_SMALL_SIZE_MAX) {
        ...
        mi_assert(heap != NULL);
        mi_assert(heap->thread_id == 0 ||
                  heap->thread_id == _mi_thread_id()); // heaps are thread local
        void *const p =
                _mi_malloc_generic(heap, size + MI_PADDING_SIZE, zero,
                                   huge_alignment); // note: size can overflow but it is
        // detected in malloc_generic
        mi_track_malloc(p, size, zero);
...
extern inline void *_mi_heap_malloc_zero(mi_heap_t *heap, size_t size,
                                         bool zero) mi_attr_noexcept {
...
mi_heap_malloc(mi_heap_t *heap, size_t size) mi_attr_noexcept {
    Printf("_mi_heap_malloc_zero %ld\n", size);
...
void *_mi_heap_realloc_zero(mi_heap_t *heap, void *p, size_t newsize,
    ...
    // else if size == 0 then reallocate to a zero-sized block (and don't return
    // NULL, just as mi_malloc(0)). (this means that returning NULL always
    // indicates an error, and `p` will not have been freed in that case.)
    const size_t size =
            _mi_usable_size(p, "mi_realloc"); // also works if p == NULL (with size 0)
    if mi_unlikely (newsize <= size && newsize >= (size / 2) &&
                    newsize > 0) { // note: newsize must be > 0 or otherwise we
        // return NULL for realloc(NULL,0)
                mi_assert_internal(p != NULL);
        // todo: do not track as the usable size is still the same in the free;
        // adjust potential padding? mi_track_resize(p,size,newsize) if (newsize <
        // size) { mi_track_mem_noaccess((uint8_t*)p + newsize, size - newsize); }
        return p; // reallocation still fits and not more than 50% waste
    }
    void *newp = mi_heap_malloc(heap, newsize);
    if mi_likely (newp != NULL) {
        if (zero && newsize > size) {
            // also set last word in the previous allocation to zero to ensure any
            // padding is zero-initialized
            const size_t start =
                    (size >= sizeof(intptr_t) ? size - sizeof(intptr_t) : 0);
            _mi_memzero((uint8_t * )
            newp + start, newsize - start);
        } else if (newsize == 0) {
            ((uint8_t * )
            newp)[0] =
            0; // work around for applications that expect zero-reallocation to be
            // zero initialized (issue #725)
        }
        if mi_likely (p != NULL) {
            const size_t copysize = (newsize > size ? size : newsize);
            mi_track_mem_defined(p, copysize); // _mi_useable_size may be too large
            // for byte precise memory tracking..
...
mi_decl_nodiscard void *mi_heap_realloc(mi_heap_t *heap, void *p,
                                        size_t newsize) mi_attr_noexcept {
    return _mi_heap_realloc_zero(heap, p, newsize, false);
}

mi_decl_nodiscard void *mi_heap_reallocn(mi_heap_t *heap, void *p, size_t count,
                                         size_t size) mi_attr_noexcept {
    size_t total;
    if (mi_count_size_overflow(count, size, &total))
        return NULL;
...
mi_decl_nodiscard void *mi_heap_reallocf(mi_heap_t *heap, void *p,
                                         size_t newsize) mi_attr_noexcept {
...
mi_decl_nodiscard void *mi_heap_rezalloc(mi_heap_t *heap, void *p,
                                         size_t newsize) mi_attr_noexcept {
...
mi_decl_nodiscard void *mi_reallocn(void *p, size_t count,
                                    size_t size) mi_attr_noexcept {
...
mi_decl_nodiscard void *mi_recalloc(void *p, size_t count,
                                    size_t size) mi_attr_noexcept {
...
mi_decl_nodiscard mi_decl_restrict char *mi_strndup(const char *s,
                                                    size_t n) mi_attr_noexcept {
...
mi_heap_realpath(mi_heap_t *heap, const char *fname,
                 char *resolved_name) mi_attr_noexcept {
  // todo: use GetFullPathNameW to allow longer file names
  char buf[PATH_MAX];
  DWORD res = GetFullPathNameA(
      fname, PATH_MAX, (resolved_name == NULL ? buf : resolved_name), NULL);
  if (res == 0) {
    errno = GetLastError();
    return NULL;
...
*/
char *mi_heap_realpath(mi_heap_t *heap, const char *fname,
                       char *resolved_name) mi_attr_noexcept {
    if (resolved_name != NULL) {
        ...
        mi_cfree(rname); // use checked free (which may be redirected to our free
        // but that's ok)
        // note: with ASAN realpath is intercepted and mi_cfree may leak the
        // returned pointer :-(
        return result;
    }
    /*
...
static bool mi_try_new_handler(bool nothrow) {
  ...
  if (h == NULL) {
    ...
    defined(__cpp_exceptions) // exceptions are not always enabled
    if (!nothrow) {
      throw std::bad_alloc();
    }
#else
...
#if (defined(__GNUC__) || \
     (defined(__clang__) && \
      !defined(_MSC_VER))) // exclude clang-cl, see issue #631
std_new_handler_t __attribute__((weak)) _ZSt15get_new_handlerv(void) {
  return NULL;
}
...
// note: on windows we could dynamically link to
...
mi_decl_nodiscard mi_decl_restrict void *mi_heap_alloc_new(mi_heap_t *heap,
                                                           size_t size) {
...
mi_heap_alloc_new_n(mi_heap_t *heap, size_t count, size_t size) {
    ...
    if mi_unlikely (mi_count_size_overflow(count, size, &total)) {
        mi_try_new_handler(false); // on overflow we invoke the try_new_handler once
        // to potentially throw std::bad_alloc
        return NULL;
    } else {
...
mi_decl_nodiscard mi_decl_restrict void *mi_new_aligned(size_t size,
                                                        size_t alignment) {
    void *p;
    do {
...
#if MI_GUARDED
// We always allocate a guarded allocation at an offset (`mi_page_has_aligned`
// will be true). We then set the first word of the block to `0` for regular
// offset aligned allocations (in `alloc-aligned.c`) and the first word to `~0`
// for guarded allocations to have a correct `mi_usable_size`

static void *mi_block_ptr_set_guarded(mi_block_t *block, size_t obj_size) {
  // TODO: we can still make padding work by moving it out of the guard page
  // area
  ...
  const size_t block_size = mi_page_block_size(
      page); // must use `block_size` to match `mi_free_local`
  const size_t os_page_size = _mi_os_page_size();
  mi_assert_internal(block_size >=
  ...
  if (segment->allow_decommit && _mi_is_aligned(guard_page, os_page_size)) {
    ...
    _mi_warning_message(
        "unable to set a guard page behind an object due to pinned memory "
        "(large OS pages?) (object %p of size %zu)\n",
        block, block_size);
  }
  ...
  if (offset > MI_BLOCK_ALIGNMENT_MAX) {
    // give up to place it right in front of the guard page if the offset is too
    // large for unalignment
...
mi_decl_restrict void *_mi_heap_malloc_guarded(mi_heap_t *heap, size_t size,
  ...
  const size_t os_page_size = _mi_os_page_size();
  const size_t obj_size = (mi_option_is_enabled(mi_option_guarded_precise)
                               ? size
                               : _mi_align_up(size, MI_MAX_ALIGN_SIZE));
  const size_t bsize = _mi_align_up(_mi_align_up(obj_size, MI_MAX_ALIGN_SIZE) +
                                        sizeof(mi_block_t),
                                    MI_MAX_ALIGN_SIZE);
  const size_t req_size = _mi_align_up(bsize + os_page_size, os_page_size);
  mi_block_t *const block = (mi_block_t *)_mi_malloc_generic(
      heap, req_size, zero, 0 /* huge_alignment */);
  if (block == NULL)
    return NULL;
