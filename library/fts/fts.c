...
fts_open(char *const *argv, int options, int (*compar)(const FTSENT **, const FTSENT **)) {
    ...
    if (!ISSET(FTS_NOCHDIR)) {
        if ((sp->fts_rfd = open(currdir(), O_RDONLY | O_CLOEXEC, 0)) == -1)
            SET(FTS_NOCHDIR);
    }
...
fts_read(FTS *sp) {
    ...
    if (instr == FTS_FOLLOW && (p->fts_info == FTS_SL || p->fts_info == FTS_SLNONE)) {
        ...
        if (p->fts_info == FTS_D && !ISSET(FTS_NOCHDIR)) {
            if ((p->fts_symfd = open(currdir(), O_RDONLY | O_CLOEXEC, 0)) == -1) {
                p->fts_errno = errno;
                p->fts_info = FTS_ERR;
            } else
...
name:
        ...
        if (p->fts_parent->fts_path[p->fts_parent->fts_pathlen - 1] != ':') {
            *t++ = '/';
        }
    ...
    if (p->fts_level == FTS_ROOTLEVEL) {
        ...
        (void) close(p->fts_symfd);
    } else if (!(p->fts_flags & FTS_DONTCHDIR) && fts_safe_changedir(sp, p->fts_parent, -1, parentdir())) {
        SET(FTS_STOP);
        return (NULL);
    }
...
fts_children(FTS *sp, int instr) {
    ...
    if ((fd = open(currdir(), O_RDONLY | O_CLOEXEC, 0)) == -1)
...
mem1:
        ...
        } else if (nlinks == 0 || (nostat && dp->d_type != DT_DIR && dp->d_type != DT_UNKNOWN)) {
    ...
    if (descend && (type == BCHILD || !nitems) && (cur->fts_level == FTS_ROOTLEVEL ? FCHDIR(sp, sp->fts_rfd) : fts_safe_changedir(sp, cur->fts_parent, -1, parentdir()))) {
        cur->fts_info = FTS_ERR;
        SET(FTS_STOP);
        return (NULL);
    }
