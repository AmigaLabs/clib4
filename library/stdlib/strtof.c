/*
 * $Id: stdlib_strtof.c,v 1.12 2006-09-25 14:51:15 clib2devs Exp $
*/
			...
			(radix == 16 && ((*str) == 'p' || (*str) == 'P')))
		{
			...
			if (exponent != 0)
			{
				if (exponent_is_negative)
				{
					...
					divisor = __stdlib_powf((float)radix, (float)exponent);
					if (divisor != 0.0)
					{
						new_sum = sum / divisor;
				...
				else
				{
					...
					new_sum = sum * __stdlib_powf((float)radix, (float)exponent);
					if (new_sum < sum)
						error = ERANGE;
					else
						sum = new_sum;
