...
/* getpwuid_r is using memalign to allocate pwd fields
...
static int __getpw_a(uid_t uid, struct passwd *pw, char **buf, size_t *size, struct passwd **res) {
    int ret = 0;
...
getpwuid_r(uid_t uid, struct passwd *pw, char *buffer, size_t bufsize, struct passwd **result) {
    ...
    int ret = __getpw_a(uid, pw, &line, &len, result);
