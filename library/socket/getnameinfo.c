/*
 * $Id: socket_getnameinfo.c,v 1.0 2021-01-13 11:31:35 clib4devs Exp $
*/
...
static void
mkptr4(char *s, const unsigned char *ip) {
    sprintf(s, "%d.%d.%d.%d.in-addr.arpa", ip[3], ip[2], ip[1], ip[0]);
}

static void
mkptr6(char *s, const unsigned char *ip) {
...
getnameinfo(const struct sockaddr *sa, socklen_t sl,
    ...
    if (node && nodelen) {
        ...
        if (!*buf && !(flags & NI_NUMERICHOST)) {
            ...
            int qlen = res_mkquery(0, ptr, 1, RR_PTR, 0, 0, 0, query, sizeof query);
            query[3] = 0; /* don't need AD flag */
            int rlen = res_send(query, qlen, reply, sizeof reply);
            buf[0] = 0;
            if (rlen > 0)
