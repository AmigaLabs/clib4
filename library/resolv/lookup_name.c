...
static int
name_from_numeric(struct address buf[static 1], const char *name, int family) {
...
static int name_from_dns(struct address buf[static MAXADDRS], char canon[static 256], const char *name, int family,
    ...
    for (i = 0; i < 2; i++) {
        if (family != afrr[i].af) {
            qlens[nq] = res_mkquery(0, name, 1, afrr[i].rr, 0, 0, 0, qbuf[nq], sizeof *qbuf);
            if (qlens[nq] == -1)
                return EAI_NONAME;
...
int
__lookup_name(struct address buf[static MAXADDRS], char canon[static 256], const char *name, int family, int flags) {
    int cnt = 0, i, j;
