/*
 * $Id: regex_regcomp.c,v 1.0 2022-03-21 18:52:18 clib2devs Exp $
*/
    ...
    while (status == REG_OK && tre_stack_num_objects(stack) > bottom) {
        ...
        switch (node->type) {
            ...
            case UNION:
                ...
                if (uni->left->nullable) STACK_PUSHX(stack, voidptr, uni->left)
                else if (uni->right->nullable) STACK_PUSHX(stack, voidptr, uni->right)
                else
                    assert(0);
                break;
...
/* Converts the syntax tree to a TNFA.	All the transitions in the TNFA are
   ...
   the regexp. */
static reg_errcode_t
tre_ast_to_tnfa(tre_ast_node_t *node, tre_tnfa_transition_t *transitions,  int *counts, int *offs) {
    tre_union_t *uni;
    tre_catenation_t *cat;
    tre_iteration_t *iter;
