...
#include "wordexp_glibc_compat.h"
...
/* clib4: no __libc_argc/__libc_argv; used only for $# / $@ expansion.
 * Provide empty defaults so ${@} and $# expand gracefully. */
static int __libc_argc;
static char **__libc_argv;
	       ...
	       const char *ifs_white)
{
  ...
  if (ifs && !*ifs)
    {
      ...
      *word = w_addstr (*word, word_length, max_length, globbuf.gl_pathv[0]);
      for (match = 1; match < (unsigned int) globbuf.gl_pathc && *word != NULL; ++match)
	{
	  *word = w_addchar (*word, word_length, max_length, ' ');
	  if (*word != NULL)
  ...
  for (match = 0; match < (unsigned int) globbuf.gl_pathc; ++match)
    {
...
/*
 * clib4: there is no fork(), so command substitution is run through
 * spawnvpe() using a POSIX shell ("sh -c <command>"), exactly like glibc
 * does with execve(_PATH_BSHELL, {"sh", "-c", comm}).  The child-side
 * redirection, exec and error handling that glibc performs in
 * exec_comm_child() are all handled inside spawnvpe(), which takes the
 * pipe fds directly (fhin/fhout/fherr).
 *
 * NOTE: a POSIX sh port must be installed (e.g. in C:) for command
 * substitution to work; it is not part of a stock AmigaOS 4 installation.
 *
 * Returns the child pid (> 0) on success, -1 on failure.
 */
static pid_t
exec_comm_spawn (char *comm, int *fildes, int showerr, int noexec)
{
  const char *args[4] = { "sh", "-c", comm, NULL };
  int fherr;
  pid_t pid;
  ...
  /* Redirect stderr to NIL: unless WRDE_SHOWERR.  */
  fherr = -1;
  if (!showerr)
    {
      fherr = open (_PATH_DEVNULL, O_WRONLY);
      if (fherr < 0)
	fherr = -1;
    }

  /* glibc unsets IFS in the child so the subshell doesn't field-split on
   * our behalf.  spawnvpe() cannot unset variables (deltaenv entries are
   * NAME=VALUE assignments), so we rely on sh defaulting IFS; a parent-set
   * IFS would still be inherited via NP_CopyVars (documented limitation).  */
  pid = spawnvpe ("C:sh", args, NULL, NULL, -1,
		  fildes[1], fherr >= 0 ? fherr : -1);
...
#define bufsize 128
  ...
  /* clib4: make the read end non-blocking.  On AmigaOS the child is a
   * separate process and, if the shell cannot be started (e.g. sh not
   * installed), it may die without ever writing to the pipe; a blocking
   * read() would then hang forever.  With O_NONBLOCK read() returns -1
   * (EAGAIN) and the waitpid(WNOHANG) below detects the dead child.  */
  {
    int fl = fcntl (fildes[0], F_GETFL, 0);
    if (fl >= 0)
      fcntl (fildes[0], F_SETFL, fl | O_NONBLOCK);
  }

 again:
  pid = exec_comm_spawn (comm, fildes, noexec ? 0 : flags & WRDE_SHOWERR,
			 noexec);
  if (pid < 0)
    {
      __close (fildes[0]);
  ...
  if (noexec)
    {
      int rc = (TEMP_FAILURE_RETRY (__waitpid (pid, &status, 0)) == pid
  ...
  if (!pwordexp)
      ...
      while (1)
	{
	  if ((buflen = TEMP_FAILURE_RETRY (__read (fildes[0], buffer,
						    bufsize))) < 1)
	    {
	      /* EAGAIN (pipe non-blocking, no data yet): if the child is
	       * still alive wait a little and retry, otherwise drain.  */
