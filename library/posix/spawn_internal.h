 ...
 */
#define PS_PORT_ACTIONS_SIZE(x) offsetof(struct _posix_spawn_port_actions, pspa_actions[(x)])
...
typedef struct _posix_spawnattr {
    ...
    int         psa_schedpolicy;            /* for get/setschedpolicy */
    _posix_spawn_port_actions_t psa_ports;  /* special/exception ports */
    struct sched_param psa_schedparam;      /* for get/setschedparam */
} *_posix_spawnattr_t;
...
typedef struct _psfa_action {
    ...
    char *psfaa_path;               /* path for chdir */
    struct _psfaa_open {
 ...
 */
#define PSF_ACTIONS_SIZE(x) offsetof(struct _posix_spawn_file_actions, psfa_act_acts[(x)])
...
struct _posix_spawn_args_desc {
    ...
    size_t                      port_actions_size;  /* size of port actions block */
    _posix_spawn_port_actions_t port_actions;       /* pointer to port block */
};

struct _posix_spawn_task_params {
    struct Task *parent;
    uint32_t startupSignal;
    const char *path;
...
int _posix_spawn_file_actions_grow(_posix_spawn_file_actions_t *psactsp);
int __posix_spawn(pid_t * __restrict pid, const char * __restrict path, struct _posix_spawn_args_desc *ad, char *const argv[ __restrict], char *const envp[ __restrict]);
