/*
 * $Id: math_asin.c,v 1.9 2021-02-16 12:04:23 clib2devs Exp $
*/
...
static const __float64
        ...
        pio4_hi = _F_64(7.85398163397448278999e-01), /* 0x3FE921FB, 0x54442D18 */
...
asin(double x) {
    ...
    if (ix >= 0x3FEF3333) { /* if |x| > 0.975 */
        w = p / q;
        t = pio2_hi - (_F_64(2.0) * (s + s * w) - pio2_lo);
    } else {
        w = s;
        SET_LOW_WORD(w, 0);
        c = (t - w * w) / (s + w);
        r = p / q;
        p = _F_64(2.0) * s * r - (pio2_lo - _F_64(2.0) * c);
        q = pio4_hi - _F_64(2.0) * w;
        t = pio4_hi - (p - q);
    }
    if (hx > 0)
        return t;
