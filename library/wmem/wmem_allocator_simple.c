...
//     int i;
//     for(i = 0; i < allocator->count; i++) DebugPrintF("[%s :] %ld : ptr [0x%lx] size [0x%lx]\n", function, i, allocator->ptrs[i], allocator->sizes[i]);
// }
...
// DebugPrintF("[wmem_simple_alloc :] Alloc : size [0x%lx]\n", size);

    if (__clib4_unlikely(allocator->count == allocator->size)) {

// DebugPrintF("[wmem_simple_alloc :] Growing ptrs array.\n");
...
// DebugPrintF("[wmem_simple_free :] ptr [0x%lx]\n", ptr);

    for (i = allocator->count; i >= 0; i--) {
        if (ptr == allocator->ptrs[i]) {
            if (i < allocator->count) {

// DebugPrintF("[wmem_simple_free :] Freeing element %d\n", i);
...
// DebugPrintF("[wmem_simple_realloc :] ptr [0x%lx] size [0x%lx]\n", ptr, size);

    for (i = allocator->count - 1; i >= 0; i--) {
        if (ptr == allocator->ptrs[i]) {
            if (size > allocator->sizes[i]) {
// DebugPrintF("[wmem_simple_realloc :] old ptr [0x%lx] old size [0x%lx]\n", allocator->ptrs[i], allocator->sizes[i]);
                // Grow
                ...
                // DebugPrintF("[wmem_simple_realloc :] Grow : new ptr [0x%lx] new size [0x%lx]\n", allocator->ptrs[i], allocator->sizes[i]);
            }
...
// DebugPrintF("[wmem_simple_realloc :] Failed to find pointer in array.\n");
    // dump_ptrs("simple_realloc", private_data);
