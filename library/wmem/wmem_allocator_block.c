...
wmem_block_verify_block(wmem_block_hdr_t *block) {
    ...
    do {
        ...
        assert(chunk->len >= WMEM_CHUNK_HEADER_SIZE);
        assert(!chunk->jumbo);

        if (WMEM_CHUNK_NEXT(chunk)) {
            assert(chunk->len == WMEM_CHUNK_NEXT(chunk)->prev);
        }
        ...
        if (!chunk->used &&
            ...
            if (!chunk->last) {
                assert(WMEM_GET_FREE(chunk)->next);
                assert(WMEM_GET_FREE(chunk)->prev);
            }
        }
    ...
    assert(total_len == WMEM_BLOCK_SIZE);
...
wmem_block_verify_master_list(wmem_block_allocator_t *allocator) {
    ...
    assert(WMEM_GET_FREE(cur)->prev == NULL);
    ...
    while (cur) {
        ...
        if (cur_free->next) {
            assert(WMEM_GET_FREE(cur_free->next)->prev == cur);
        }

        if (cur != allocator->master_head) {
            assert(cur->len == WMEM_BLOCK_SIZE);
        }
...
wmem_block_verify_recycler(wmem_block_allocator_t *allocator) {
    ...
    do {
        ...
        assert(cur_free->prev);
        assert(cur_free->next);

        assert(WMEM_GET_FREE(cur_free->prev)->next == cur);
        assert(WMEM_GET_FREE(cur_free->next)->prev == cur);
...
wmem_block_verify(wmem_allocator_t *allocator) {
    ...
    assert(allocator->type == WMEM_ALLOCATOR_BLOCK);

    private_allocator = (wmem_block_allocator_t *) allocator->private_data;

    if (private_allocator->block_list == NULL) {
        assert(!private_allocator->master_head);
        assert(!private_allocator->recycler_head);
        return;
    }
    ...
    assert(chunk_free == master_free + recycler_free);
}
...
wmem_block_split_free_chunk(wmem_block_allocator_t *allocator,
    ...
    aligned_size = WMEM_CHUNK_HEADER_SIZE + alignment + sizeof(wmem_block_pre_t) + size;
    aligned_size = WMEM_ALIGN_SIZE(aligned_size);

    if (chunk->len < aligned_size + WMEM_CHUNK_HEADER_SIZE + sizeof(wmem_block_free_t)) {
...
static void *
wmem_block_alloc_jumbo(wmem_block_allocator_t *allocator, const size_t size, const size_t alignment) {
    wmem_block_hdr_t *block;
    wmem_block_chunk_t *chunk;
...
wmem_block_alloc(void *private_data, const size_t size, int32_t alignment) {
    ...
    if (allocator->recycler_head && allocator->recycler_head->len >= needed) {
        ...
        if (allocator->master_head && allocator->master_head->len < needed) {
    ...
    wmem_block_split_free_chunk(allocator, chunk, size, alignment);
...
wmem_block_realloc(void *private_data, void *ptr, const size_t size, int32_t alignment) {
    ...
    if (new_ptr_supremum > chunk_supremum) { // Let's assume that alignment is the same for old and new chunk
        ...
        if (tmp && (!tmp->used) &&
            ...
            wmem_block_split_free_chunk(allocator, tmp, split_size, 0);
