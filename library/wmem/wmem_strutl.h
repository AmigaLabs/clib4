...
extern char * wmem_strdup(wmem_allocator_t *allocator, const char *src) __attribute__((__malloc__));

#define ws_strdup(src) wmem_strdup(NULL, src)

extern char * wmem_strndup(wmem_allocator_t *allocator, const char *src, const size_t len) __attribute__((__malloc__));

#define ws_strndup(src, len) wmem_strndup(NULL, src, len)

extern char * wmem_strdup_printf(wmem_allocator_t *allocator, const char *fmt, ...) __attribute__((__malloc__)) __attribute__((__format__ (__printf__, 2, 3)));

#define ws_strdup_printf(...) wmem_strdup_printf(NULL, __VA_ARGS__)

extern char * wmem_strdup_vprintf(wmem_allocator_t *allocator, const char *fmt, va_list ap) __attribute__((__malloc__));
