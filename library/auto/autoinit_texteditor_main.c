/*
**	$Id$
**	Generated by IDLTool 53.31
**	Do not edit
**
**	AutoInit stub for texteditor
**
**	Copyright (c) 2010 Hyperion Entertainment CVBA.
**	All Rights Reserved.
*/

#include <intuition/intuition.h>
#include <intuition/classes.h>

#include <interfaces/texteditor.h>
#include <proto/exec.h>
#include <proto/dos.h>
#include <proto/intuition.h>
#include <stdlib.h>

static struct Library * __IntuitionBase = NULL;
static struct IntuitionIFace *__IIntuition = NULL;
struct TextEditorIFace * ITextEditor = NULL;
static struct ClassLibrary * __TextEditorBase = NULL;
static struct TextEditorIFace * __ITextEditor = NULL;

/****************************************************************************/

extern struct ClassLibrary * TextEditorBase;
extern Class * TextEditorClass;

/****************************************************************************/

void texteditor_main_constructor(void)
{
    if (ITextEditor != NULL)
    {
        return; /* Someone was quicker */
    }
    if (TextEditorBase == NULL) /* Library base is NULL, we need to open it */
    {
        if (IIntuition == NULL)
        {
            if (IntuitionBase == NULL)
            {
                __IntuitionBase = IntuitionBase = IExec->OpenLibrary("intuition.library", 0);
                if (IntuitionBase == NULL)
                {
                    BPTR StdErr = IDOS->ErrorOutput();
                    if (StdErr == ZERO)
                        StdErr = IDOS->Output();
                    IDOS->FPrintf(StdErr, "ERROR: Failed to open '%s'.\n", "intuition.library");
                    abort();
                }
            }
            __IIntuition = IIntuition = (struct IntuitionIFace *)IExec->GetInterface(IntuitionBase, "main", 1, NULL);
            if (IIntuition == NULL)
            {
                BPTR StdErr = IDOS->ErrorOutput();
                if (StdErr == ZERO)
                    StdErr = IDOS->Output();
                IDOS->FPrintf(StdErr, "ERROR: Failed to get '%s' V%ld interface for '%s'.\n", "main", 1, "intuition.library");
                abort();
            }
        }
        /* We were called before the base constructor.
         * This means we will be called _after_ the base destructor.
         * So we cant drop the interface _after_ closing the last library base,
         * we just open the library here which ensures that.
         */
        __TextEditorBase = TextEditorBase = IIntuition->OpenClass("gadgets/texteditor.gadget", 0, &TextEditorClass);
        if (TextEditorBase == NULL)
        {
            BPTR StdErr = IDOS->ErrorOutput();
            if (StdErr == ZERO)
                StdErr = IDOS->Output();
            IDOS->FPrintf(StdErr, "ERROR: Failed to open '%s'.\n", "gadgets/texteditor.gadget");
            abort();
        }
    }

    __ITextEditor = ITextEditor = (struct TextEditorIFace *)IExec->GetInterface((struct Library *)TextEditorBase, "main", 1, NULL);
    if (ITextEditor == NULL)
    {
        BPTR StdErr = IDOS->ErrorOutput();
        if (StdErr == ZERO)
            StdErr = IDOS->Output();
        IDOS->FPrintf(StdErr, "ERROR: Failed to get '%s' V%ld interface for '%s'.\n", "main", 1, "gadgets/texteditor.gadget");
        abort();
    }
}
__attribute__((section(".ctors.zzzy"))) static void
(*texteditor_main_constructor_ptr)(void) USED = texteditor_main_constructor;

/****************************************************************************/

void texteditor_main_destructor(void)
{
    if (__ITextEditor)
    {
        IExec->DropInterface ((struct Interface *)__ITextEditor);
    }
    if (__TextEditorBase != NULL)
    {
        IIntuition->CloseClass(__TextEditorBase);
    }
    if (__IIntuition != NULL)
    {
        IExec->DropInterface((struct Interface *)__IIntuition);
    }
    if (__IntuitionBase != NULL)
    {
        IExec->CloseLibrary(__IntuitionBase);
    }
}
__attribute__((section(".dtors.zzzy"))) static void
(*texteditor_main_destructor_ptr)(void) USED = texteditor_main_destructor;

/****************************************************************************/

