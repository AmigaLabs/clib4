...
void bench_impl(const std::vector<memcpy_ty *> &toTest, unsigned size,
    ...
    for (auto handle: toTest) {
        uint64_t res = measure(handle, dest_ptr, src_ptr, size);
        std::cout << res << ", ";
    }
    std::cout << std::endl;
}
...
int main(int argc, char **argv) {
    ...
    std::vector < memcpy_ty * > toTest = {
            &libc_memcpy, &memcpy, &local_memcpy, &nop};

    std::cout << "Batches of random sizes:\n";
    std::cout << "libc@plt, libc, c_memcpy, nop,\n";

    bench_rand_range(toTest);

    std::cout << "\nFixed size:\n";
    std::cout << "size, libc@plt, libc, c_memcpy, nop,\n";
