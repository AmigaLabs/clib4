...
summarize() {
    ...
    if (debug_dump) {
        for (mx = 0; mx < malloc_calls_made; mx++) {
            printf("Buffer index %d: Address = 0x%llx, Size=%d, Fill=%d ", mx,
                   (unsigned long) (minfo_array[mx].buf_ptr),
...
check_overlap(void *p1, int len1, void *p2, int len2) {
    ...
    bob1 = (unsigned long) p1;
    eob1 = bob1 + (unsigned long) len1 - 1LL;

    bob2 = (unsigned long) p2;
    eob2 = bob2 + (unsigned long) len2 - 1LL;
    ...
    for (;;) {
                ...
                if (!(minfo_array[random_buf].this_buffer_freed)) {
                    ...
                    if (signal_happened) {
                        ...
                        printf("FAIL:  Signal %d caught during free() of buffer 0x%llx\n", signal_happened,
                               (unsigned long) (minfo_array[random_buf].buf_ptr));
                        exit(1);
                    }
                    ...
                    if (debug_dump) {
                        printf("INFO: Freed buffer index %d, (%d bytes) at address 0x%llx\n", random_buf,
                               minfo_array[random_buf].buf_size,
                               (unsigned long) (minfo_array[random_buf].buf_ptr));
                    }
        ...
        if (signal_happened) {
            printf("\nFAIL:  Signal %d occurred storing numeric types at address 0x%llx (%d bytes)\n", signal_happened,
                   (unsigned long) malloc_buf, buf_size);
            summarize();
            exit(1);
        }
